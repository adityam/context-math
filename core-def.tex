...
\ifnum\texengine=\luatexengine \else
    %let \initializeluainstances \relax
    \let \MPLIBallocate          \gobbleoneargument
    \let \MPLIBregister          \relax
    \let \xmlinitialize          \relax
\fi
...
\ifdefined\resetallattributes
    \prependtoks  \resetallattributes\to \everybeforeoutput
\fi

% in mkiv we no longer have encodings, so ... at some point (when math is redone)
% we will introduce \definemathortextcommand which will manage these things

\ifnum\texengine=\luatexengine
    \ifx\in   \undefined\else \let\normalmathin   \in    \def\in   {\mathortext\normalmathin   \dospecialin   } \fi
    \ifx\at   \undefined\else \let\normalmathat   \at    \def\at   {\mathortext\normalmathat   \dospecialat   } \fi
    \ifx\about\undefined\else \let\normalmathabout\about \def\about{\mathortext\normalmathabout\dospecialabout} \fi
    \ifx\from \undefined\else \let\normalmathfrom \from  \def\from {\mathortext\normalmathfrom \dospecialfrom } \fi
    \ifx\over \undefined\else \let\normalmathover \over  \def\over {\mathortext\normalmathover \dospecialabout} \fi
\fi
